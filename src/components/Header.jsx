...
const Header = ({
  ...
  return (
    <div className="bg-[#f7fdfd] md:mt-24">
      <div
      ...
      >
        <div className="w-full z-10">
          <div className=" md:mt-6 mt-6">
            <p className="md:text-white text-blue-900   font-bold text-4xl">{title}</p>
          </div>
          <br />br

          <div className="w-full flex items-center justify-around bg-fuchsia-50 px-2 md:px-4 py-2 md:py-3 md:mb:32 shadow-2xl rounded-full">
            <SearchInput
          ...
          {type && (
            <div className="w-full lg:1/2 flex flex-wrap gap-3 md:gap-6 py-10 md:py-14">
              {popularSearch.map((search, index) => (
                <span
                  key={index}
                  className="bg-[#101011e6] md:text-[#eeeff1] text-white py-1 px-2 rounded-full text-sm md:text-base"
                >
                  {search}
                </span>
              ))}
        ...
        <div className="w-full h-full absolute top-28 md:-top-6 lg:-top-14 right-16 left-2">
          <img
            src={HeroImage}
            className="object-contain"
            style={{ width: "1550px", height: "360px", objectFit: "fill" }}
          />
        </div>
      </div>
    </div>
